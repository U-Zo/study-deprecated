...
public class OracleMybatisDAO {
    public List<Dept> selectTopN(SqlSession session, int offset, int limit) {
        List<Dept> list = null;
        list = session.selectList("com.dept.DeptMapper5.topN", null, new RowBounds(offset, limit));
    ...
    public void multiInsert(SqlSession session, List<Dept> depts) {
        int n = 0;
        n = session.insert("com.dept.DeptMapper4.multiInsert", depts);

        System.out.println("삽입된 레코드: " + n);
    }

    public void multiDeleteDept(SqlSession session, List<Dept> depts) throws RecordNotFoundException {
        int n = 0;
        n = session.delete("com.dept.DeptMapper4.multiDeleteDept", depts);
        if (n == 0) throw new RecordNotFoundException("삭제할 데이터가 없습니다.");
        System.out.println("삭제된 레코드: " + n);
    }

    public void multiDelete(SqlSession session, List<String> locs) throws RecordNotFoundException {
        int n = 0;
        n = session.delete("com.dept.DeptMapper4.multiDelete", locs);
        if (n == 0) throw new RecordNotFoundException("삭제할 데이터가 없습니다.");
        System.out.println("삭제된 레코드: " + n);
    }

    public void multiUpdate(SqlSession session, List<Integer> deptnos) throws RecordNotFoundException {
        int n = 0;
        n = session.update("com.dept.DeptMapper4.multiUpdate", deptnos);
        if (n == 0) throw new RecordNotFoundException("수정할 데이터가 없습니다.");
        System.out.println("수정된 레코드: " + n);
    }

    public List<Dept> multiSelect(SqlSession session, List<Integer> deptnos) {
        List<Dept> list = session.selectList("com.dept.DeptMapper4.multiSelect", deptnos);
    ...
    public List<Dept> selectDynamicDeptno(SqlSession session, HashMap<String, Integer> map) {
        // deptno 의 존재에 따라 동적으로 sql 을 다르게 수행함
        List<Dept> list = session.selectList("com.dept.DeptMapper3.selectByDynamicDeptno", map);
    ...
    public List<Dept> selectDynamicDeptno2(SqlSession session, Dept dept) {
        // deptno 의 존재에 따라 동적으로 sql 을 다르게 수행함
        List<Dept> list = session.selectList("com.dept.DeptMapper3.selectByDynamicDeptno2", dept);
